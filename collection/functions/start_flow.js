 ...
 * @callback callback event callback handler.
 */
    ...
    if (participant) {
        ...
        if (status == "Prompted") {
            ...
            await handlePromptResponse(
                context, lastPrompt, event['MediaUrl0'], participant, promptHelper);
        } else if (status == "Ready" || status == "Consented") {
            // Send the first image for consented and ready users.
            await handleSendPrompt(context, participant, promptHelper);
        }
 ...
 * @returns 
 */
async function handlePromptResponse(context, lastPrompt, mediaUrl, participant) {
    ...
    let proceed = await uploadHelper.uploadVoice(
        context, lastPrompt, mediaUrl, participant, promptHelper);
    if (proceed) {
        // Send next prompt.
        await handleSendPrompt(context, participant, promptHelper);
    }
